Feature: Image Serving API Endpoints
    ...
    Scenario: Image serving with non-existent image
        Given a collection exists but the specified image ID does not exist
        When the client requests GET /api/images/:collectionId/:imageId
        Then the API returns 404 status code
        And the API returns error message indicating image not found

    Scenario: Thumbnail serving with missing thumbnail file
        Given a collection exists with an image but no thumbnail file
        When the client requests GET /api/images/:collectionId/:imageId/thumbnail
        Then the API returns 404 status code
